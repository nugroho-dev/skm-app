...
class ReportServiceController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($services as $service) {
            ...
            $reportPerService[] = [
                'service' => $service,
                'respondents_count' => $serviceRespondents->count(),
        ...
        return view('dashboard.reportservice.index', compact(
            'unsurs',
            'institutions',
            'quarters',
            'semesters',
    ...
    public function reportPerService(Request $request)
{
    $title = 'Laporan SKM';
    $subtitle = 'Rincian Laporan SKM Per Jenis Layanan';

    $unsurs = Unsur::orderBy('label_order')->get();

    // ambil id layanan dari link tombol
    $serviceId = $request->service_id;
    $service = Service::findOrFail($serviceId);

    // ambil responden hanya untuk service ini
    $query = Respondent::with(['answers.question.unsur', 'service'])
    ...
    return view('dashboard.reportservice.service_detail', compact(
        'respondents',
        'unsurs',
        'quarters',
        'semesters',
        ...
        'nilaiSKM',
        'kategoriMutu',
        'service',// supaya bisa tampil nama layanan di view
...
public function cetakPdf(Request $request)
    ...
    // ambil data yang sama seperti cetak()
        ...
        $pdf = Pdf::loadView('dashboard.reportservice.cetak_pdf', $data)
    ...
    private function getCetakData(Request $request)
    {
        ...
        return compact('respondents', 'unsurs','quarters','semesters', 'months', 'years', 'title', 'subtitle','totalPerUnsur','respondentScores','averagePerUnsur','weightedPerUnsur', 'totalBobot', 'nilaiSKM', 'kategoriMutu','service','totalRespondents', 'genderCounts', 'educationCounts', 'educationNames','occupationCounts', 'occupationNames');
    ...
    // ambil data yang sama seperti cetak()
        ...
        $pdf = Pdf::loadView('dashboard.reportservice.publikasi_pdf', $data)
